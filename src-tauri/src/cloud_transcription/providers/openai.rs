use crate::cloud_transcription::models::DEFAULT_OPENAI_BASE_URL;
...
pub async fn transcribe_with_openai(
    ...
    let provider_record = db
        .get_cloud_provider("openai")
        .map_err(|e| format!("Database error: {}", e))?
        .ok_or_else(|| {
            format!(
                "Cloud provider 'openai' is not configured. Please add an API key in Models settings."
            )
        })?;
    ...
    if api_key.trim().is_empty() {
        return Err(format!(
            "API key for 'openai' is missing. Please configure it in Models."
        ));
    }
