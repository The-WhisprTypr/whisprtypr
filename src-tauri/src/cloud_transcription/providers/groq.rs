use crate::cloud_transcription::models::DEFAULT_GROQ_BASE_URL;
use crate::cloud_transcription::util::build_http_client;
use crate::database::Database;
use reqwest::multipart::{Form, Part};

pub async fn test_groq_connection(api_key: &str, base_url: Option<&str>) -> Result<String, String> {
...
pub async fn transcribe_with_groq(
    ...
    let provider_record = db
        .get_cloud_provider("groq")
        .map_err(|e| format!("Database error: {}", e))?
        .ok_or_else(|| {
            format!(
                "Cloud provider 'groq' is not configured. Please add an API key in Models settings."
            )
        })?;
    ...
    if api_key.trim().is_empty() {
        return Err(format!(
            "API key for 'groq' is missing. Please configure it in Models."
        ));
    }
