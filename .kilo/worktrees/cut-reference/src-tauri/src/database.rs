    ...
    pub fn get_settings(&self) -> Result<AppSettings> {
        ...
        conn.query_row(
            "SELECT push_to_talk_key, toggle_key, hotkey_mode, language, selected_model_id,
                    ...
                    post_processing_enabled, voice_commands_enabled, clipboard_mode, custom_vocabulary
             FROM settings WHERE id = 1",
            [],
            |row| {
                let vocab_json: String = row.get(13).unwrap_or_else(|_| "[]".to_string());
